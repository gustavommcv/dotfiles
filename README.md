...
- **`notebook`** *(this branch)*: Dedicated to my **Notebook** configuration — see [hypr/README.md](hypr/README.md) for hardware-specific notes (touchpad gestures, battery/idle timers, dark-theme forcing for Qt/GTK apps).
...
cd ~/dotfiles
git checkout notebook   # or: main / wsl, depending on the machine — see Branches above
...
LSP servers/formatters regardless of Bun, since Neovim spawns them outside any login shell). Kept
separate so the editor config can be versioned and updated on its own; see that repo for plugins,
keymaps, and full dependency docs. Only clones if `~/.config/nvim` doesn't already exist —
re-running `install.sh` never overwrites local edits there.
